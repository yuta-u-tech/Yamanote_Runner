...
実機検証時は、Xcode Scheme の Environment Variables に以下を設定すると、Paywallの管理者フォームからマップ機能を解放できる。

- `YAMANOTE_ADMIN_EMAIL`
- `YAMANOTE_ADMIN_PASSCODE`

入力値が一致すると `vol2.adminSubscriptionOverride` が `true` で保存され、以後はStoreKitの購入状態に関係なく購読済み扱いになる。
