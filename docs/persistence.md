...
| データ | 保存キー案 | 用途 |
| --- | --- | --- |
| 開発者向け購読アクセス | `vol2.developerSubscriptionAccess` | 開発者確認時にStoreKit購入状態とは別にアプリ全体を解放 |
...
開発ビルドの実機検証時は、通常のPaywallにある「購入を復元する」経由で渡される環境変数によりアプリ全体を解放できる。使用する環境変数は以下。

- `YAMANOTE_ADMIN_EMAIL`
- `YAMANOTE_ADMIN_PASSCODE`

開発ビルドで両方の値が空でない場合のみ、`vol2.developerSubscriptionAccess` が `true` で保存され、以後はStoreKitの購入状態に関係なく購読済み扱いになる。本番ビルドではこの経路は含めず、通常のApp Store購入/復元のみを利用する。
