...
実機検証時は、Xcode Scheme の Environment Variables に以下を設定すると、通常のPaywallにある「購入を復元する」からマップ機能を解放できる。

- `YAMANOTE_ADMIN_EMAIL`: `yuta_Hinu_auth@email.com`
- `YAMANOTE_ADMIN_PASSCODE`: `yuta_Hinu_pass`

環境変数が一致すると `vol2.adminSubscriptionOverride` が `true` で保存され、以後はStoreKitの購入状態に関係なく購読済み扱いになる。この設定は本番デプロイ前に削除する。
